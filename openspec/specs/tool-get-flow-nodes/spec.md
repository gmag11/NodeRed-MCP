...
The system SHALL expose an MCP tool named `get-flow-nodes` that accepts a `flowId` (required) and returns a paginated list of nodes within that flow, with metadata and sanitized configuration.

#### Scenario: Retrieve nodes from a known flow
- **WHEN** an MCP client invokes `get-flow-nodes` with `flowId: "abc"`
- **THEN** the system returns all nodes whose `z` property equals `"abc"`, including each node's `id`, `type`, `name`, `disabled` state, `x`/`y` position, and `wires`
...
The `get-flow-nodes` tool SHALL include node configuration fields in the response, excluding known large text fields (`func`, `template`, `format`, `html`, `css`) to avoid wasting LLM context tokens. All other configuration fields SHALL be preserved.
...
The tool SHALL accept an optional `nodeType` string parameter. When provided, only nodes whose `type` matches the value SHALL be returned.
