...
The system SHALL expose an MCP tool named `search-nodes` that accepts `name` (optional string, substring match), `type` (optional string, substring match), `property` (optional string, property name), `value` (optional string, property value for equality match), and `limit` (optional number, default 50). At least one filter must be provided. It SHALL search all regular nodes across all flows and return matching results.
