...
- [ ] In `src/server.js`, change `get-palette-nodes` input schema:
...
- [ ] In `src/server.js`, update `get-palette-nodes` description text to use offset/limit terminology instead of page/pageSize

## 3. Update the handler
- [ ] In `src/tools/get-palette-nodes.js`:
...
## 4. Update tests
- [ ] In `tests/tools/get-palette-nodes.test.js`:
...
- [ ] Manual check: call `get-palette-nodes` with `offset=0, limit=10` and verify response
