# skill-resources Delta Specification
...
The system SHALL expose a `list-skills` MCP tool that returns an array of all available skills from `resources/skills/`, each with `name`, `description`, and `uri` fields.
...
**Reason**: The `get-skill` tool has since been added as a fallback for MCP clients that do not support the Resources capability. This requirement is obsolete.

**Migration**: None required. The tool already exists in the codebase.

### Requirement: No prompts for skills

**Reason**: Skill prompts have been removed in a prior change. This requirement no longer applies.
