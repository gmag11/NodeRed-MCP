...
The system SHALL expose an MCP tool named `remove-nodes-from-group` that accepts `groupId` (required string) and optional `nodeIds` (array of strings) and `reposition` (boolean, default `false`). The tool SHALL detach the specified nodes from the group and optionally reposition them outside the group's bounding rectangle.
...
#### Scenario: Reposition multiple nodes in a column
- **WHEN** `remove-nodes-from-group` is invoked with `reposition: true` and multiple `nodeIds`
- **THEN** nodes are placed in a vertical column to the right of the group, spaced 40px apart vertically
...
- **THEN** the node is silently skipped (no error), but a warning is included in the response
...
The tool SHALL return `groupId`, `removedNodeIds` (the nodes that were actually removed), `remainingNodeIds` (nodes still in the group), and `repositionedNodes` (array of `{ nodeId, x, y }`) in its response.
